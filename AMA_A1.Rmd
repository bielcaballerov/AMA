...
b_max_log_lik <- function(b_set, x){
  ...
  for(b in b_set){
    ...
    hx_f <- stepfun(hx$breaks,c(0,hx$density,0))
    hx_f2 <- (n/(n-1)* hx_f(x))- 1/((n-1)*b) #(n/(n-1)* hx_f_i(x)) * 1/((n-1)*b) # 1/(n-1)*(n*hx_f_i(x) - (1/b)) #(n/(n-1)*hx_f_i(x)) - 1/((n-1)*b)
    hx_f2[abs(hx_f2) < 1e-05] <- 0
  ...
  plot(x=seq(1,30),log_liks_b, type = "p" )
...
hx <- hist(x, breaks=seq(A, Z, by=b), plot=F)
plot(hx, freq = FALSE)
...
nbr_opt <- h_set[which.max(log_liks_h)]
hist(x,breaks=seq(A,Z,length=nbr_opt+1),freq=F)
```
