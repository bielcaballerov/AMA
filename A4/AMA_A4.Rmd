...
```
Looking at the plot of the data we can see that the varicance is not constant for all x. The variance at the beginning seems to be lower.
...
```
Here we can see that the residual of the estimation and the datapoints have a high variance that increases over x until abou x=68 and decreases again from there. This also shows the changing variance which is also shown in the blue line. While the variance is close to zero at the beginning it rises to about 1.
...
```
The result looks almost the same to the result of the former Local polynomial regression function.
...
```
Also this result looks very similar to the result of the former Local polynomial regression function.
