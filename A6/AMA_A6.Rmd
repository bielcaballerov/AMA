...
```
The optimal number of degree of freedoms obtained by k-fold log-likelihood cross validation is df=5.
...
print(sprintf("min: %f,  max: %f", x.min, x.max))
```
In comparison to the previous result, the optimised spline looks smoother while having almost the same range of x [20.7, 32.6] for which Pr(cnt >= 5000|temp) larger than 0.5.
