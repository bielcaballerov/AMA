...
```
```{r}
gam_6.1 <- gam(FGm12 ~ SysPres + s(FGm0,SysPres, by=Treatment, k=10) + te(height, k=25, bs="cr") + te(weight, bs="cr") + ti(height, weight))
summary(gam_6.1)
```

```{r}
gam_6.1 <- gam(FGm12 ~ SysPres + s(FGm0,SysPres, by=Treatment, k=10) + s(height, k=25, bs="cr") + s(weight, bs="cr"))
summary(gam_6.1)
```
Since the interaction is taking into account the different factors from Treatment we had a lot of uncertaintly
```{r}
