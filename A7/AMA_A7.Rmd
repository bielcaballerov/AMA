...
```
```{r} 
gam_1.2 <- gam(FGm12 ~ s(FGm0) + s(SysPres) + s(DiaPres) + s(height) + s(weight), data=hirs)
summary(gam_1.2)
```
It seems that SysPres and height are both linear.
...
```{r}
# semiparametric model - FGmO
gam_3.2 <- gam(FGm12 ~ FGm0 + s(SysPres) + s(DiaPres) + s(height) + s(weight))
summary(gam_3.2)
```
...
gam_4.1 <- gam(FGm12 ~ s(FGm0, by=Treatment) + SysPres + s(DiaPres, by=Treatment) + s(height, by=Treatment) + s(weight, by=Treatment))
summary(gam_4.1)
```
```{r}
...
gam_4.2 <- gam(FGm12 ~ FGm0 + s(SysPres, by=Treatment) + s(DiaPres, by=Treatment) + s(height, by=Treatment) + s(weight, by=Treatment))
summary(gam_4.2)
```
```{r}
plot(gam_4.2,residuals=TRUE, shade=TRUE, shade.col="lightblue", seWithMean=TRUE, cex=3, lwd=2, shift= coef(gam_4.1)[1])
```
```{r}
gam.check(gam_4.2)
```
...
gam_4.3 <- gam(FGm12 ~ s(FGm0, by=Treatment) + s(SysPres, by=Treatment) + DiaPres + s(height, by=Treatment) + s(weight, by=Treatment))
summary(gam_4.3)
```
```{r}
plot(gam_4.3,residuals=TRUE, shade=TRUE, shade.col="lightblue", seWithMean=TRUE, cex=3, lwd=2, shift= coef(gam_4.3)[1])
```
```{r}
gam.check(gam_4.3)
```
...
gam_5.2 <- gam(FGm12 ~ FGm0 + s(SysPres, by=Treatment) + DiaPres + s(height, by=Treatment) + s(weight, by=Treatment))
summary(gam_5.2)
```
```{r}
plot(gam_5.2,residuals=TRUE, shade=TRUE, shade.col="lightblue", seWithMean=TRUE, cex=3, lwd=2, shift= coef(gam_5.2)[1])
```
```{r}
gam.check(gam_5.2)
```
...
```{r}
gam_6.1 <- gam(FGm12 ~ FGm0 + SysPres + s(DiaPres, by=Treatment) + s(height, by=Treatment) + s(weight))
summary(gam_6.1)
```
```{r}
...
```{r}
gam_7.1 <- gam(FGm12 ~ FGm0 + SysPres + ti(DiaPres,weight) + s(height, by=Treatment))
summary(gam_7.1)
```
```{r}
plot(gam_7.1,residuals=TRUE, shade=TRUE, shade.col="lightblue", seWithMean=TRUE, cex=3, lwd=2, shift= coef(gam_7.1)[1])
```
```{r}
gam.check(gam_7.1)
```
...
```
```{r}
vis.gam(gam_6.1,view=c("FGm0","Treatment"),
        theta = 40, phi = 40, r = sqrt(3), d = 1,)
vis.gam(gam_6.1, view=c("FGm0","Treatment"), plot.type = "persp", theta=40, phi=40)
vis.gam(gam_6.1, view=c("FGm0","weight"), plot.type = "contour")
```
...
```{r}
anova(gam_4.2,gam_4.3,test="F")
```
Gam_4.2 better

```{r}
anova(gam_4.2,gam_1.1,test="F")
```
Gam_1.1 better

```{r}
anova(gam_5.1,gam_5.2,test="F")
```
Gam_5.1 better

```{r}
anova(gam_5.1,gam_1.1,test="F")
```
Gam_5.1 better


```{r}
anova(gam_5.1,gam_6.1)
```

Gam_6.1 better

```{r}
anova(gam_7.1, gam_6.1)
```


```{r}
anova(gam_2.1, gam_3.2, gam_4.3, gam_5.2, gam_3.1, gam_5.1, gam_6.1, test="F")
```
