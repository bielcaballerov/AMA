...
loglik.CV <- function(x, y, h) {
    ...
    # return(-sum( log(exp(-lambda)*((lambda^y)/factorial(y))))/n )
    return(-sum(-lambda + log(lambda^y/factorial(y)))/n )
}
```
...
```
The leave-one-out cross-validation determines 2.61 as optimal bandwidth. The obtained fit of the local Poisson regression model looks reasonable.