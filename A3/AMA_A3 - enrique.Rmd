...
```
We've selected 9 points that tries to cover the variability of the data, this points are marked in red and each point has assigned a number in order to recognized each point with its representation using the *plot.zip* function.
...
text(aux2[,1],aux2[,2],1:9, pos=3, col=2, font = 2, cex = 1.2)
```

We can notice 2 patterns in the printed zeros below. The first one (the y-axis) is related to thickness of the stroke, positive values are thicker (the higher the value, the higher the stroke) and viceversa (the lower the value, the lower the stroke). On the oher hand, the x-axis could be related to the roundness of the number (negative values are like an oval, positive values are like a circle). Furthermore, it also seems that positive values in x tends to have unfinished strokes, like fast writing.
