...
# create arrays for loo
X_loo <- array(0, dim=c(N-1, 3, N))
pt_loo <- array(0, dim=c(1, 3, N))
...
# calculate avg sum-of-squared distances of loo proncipal curves for each df 
...
for(i in df){
  sum_dist <- 0
  for (n in 1:N){
    fit_in <- principal_curve(X_loo[, ,n], df=i)
    #print(pt_loo[, ,n])
    #print(matrix(pt_loo[, ,n],ncol=3,byrow=TRUE))
    loo.proj <- project_to_curve(matrix(pt_loo[, ,n],ncol=3,byrow=TRUE), fit_in$s[fit_in$ord,])
    #print(loo.proj$dist)
    sum_dist <- sum_dist + loo.proj$dist
  }
  avg_dists[i-1] <- sum_dist/N
}
